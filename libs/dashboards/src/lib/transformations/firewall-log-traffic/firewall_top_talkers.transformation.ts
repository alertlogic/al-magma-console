...
/**
 * topTalkers transformation
 * kalm: dashboards_w_top_talkers, dashboards_fw_total_connections
 ...
 */
export const topTalkers = (response: {rows:any[], column_info:any[]}[]): TableListConfig | ZeroState => {
  ...
  return {
    ...
    body: assets.map((asset) => ({
      ...
      percentage_connections_count: `${((asset[Column.ConnectionsCount] * 100) / totalConnections).toFixed(2)}%`
    }))
  };
};
