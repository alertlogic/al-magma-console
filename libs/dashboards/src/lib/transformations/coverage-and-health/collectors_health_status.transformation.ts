...
export const collectorsHealthStatus = (response: HealthSummaryResponse, item?: WidgetConfig) => {
  const config = Object.assign({}, baseSemiCircleObject);
  const healthy = response.collectors.health.scores[0].count;
  const unhealthy = response.collectors.health.scores[1].count;
