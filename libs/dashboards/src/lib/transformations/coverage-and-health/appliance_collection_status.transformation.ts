...
export const applianceCollectionStatus = (response: HealthSummaryResponse, item?: WidgetConfig) => {
  const config = Object.assign({}, baseSemiCircleObject);
  const healthy = response.appliances.health.scores[0].count;
  const unhealthy = response.appliances.health.scores[1].count;
