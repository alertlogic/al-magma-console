...
export const agentCollectionStatus = (response: HealthSummaryResponse, item?: WidgetConfig) => {
  const config = Object.assign({}, baseSemiCircleObject);
  const healthy = response.agents.health.scores[0].count;
  const unhealthy = response.agents.health.scores[1].count;
