...
@Component({
    selector: 'al-notification-health-alert-form',
    templateUrl: './al-notification-health-alert-form.component.html',
    styleUrls: ['./al-notification-health-alert-form.component.scss']
})
export class AlNotificationHealthAlertFormComponent implements OnInit {
    ...
    readonly footerOptions: AlBottomSheetFooterOptions = {
        primaryAction: {
            text: 'Next',
            disabled: true,
            hidden: false
    ...
    onBackAction(): void {
        this.alWizard.stepBack();
        this.validateBackStateButton();
        this.validateStep(this.alWizard.stepIndex);
    }
    ...
    validateStep(stepIndex: number): void {
        ...
        switch (stepIndex) {
            case 0:
                validStep = this.validateFirstStep();
                break;
            case 1:
                validStep = this.validateSecondStep();
                break;
            case 2:
                validStep = this.validateThirdStep();
                break;
            default:
                break;
        }
        this.setPrimaryActionState(stepIndex === 2 ? this.validateAllSteps(): validStep);
        this.checkPrimaryButtonText(stepIndex);
        this.alWizard.steps[stepIndex].complete = validStep;
    }

    public validateAllSteps(): boolean {
        const allValid: boolean = this.validateFirstStep()&&this.validateSecondStep()&&this.validateThirdStep();
        if (allValid && this.alWizard.stepIndex === 2) {
            this.alWizard.steps.forEach(step => {
                step.complete = true;
            });
        }
        this.alWizard.steps[0].complete = this.validateFirstStep();
        this.alWizard.steps[1].complete = this.validateSecondStep();
        this.alWizard.steps[2].complete = this.validateThirdStep();
        this.setPrimaryActionState(allValid);
        return allValid;
    }
    ...
    private setAssetFilters(assets: AssetFilterOption[]) {
        this.formData.selectedAssets = assets.map(
            asset => {
                const deploymentId: string | undefined = asset.hasOwnProperty('asset.deployment_id') ? asset['asset.deployment_id'] : undefined;
                const assetType: string = deploymentId ? "deployment" : asset.hasOwnProperty('asset.vpc') ? "vpc" : "subnet";
                let assetKey: string | undefined = (asset['asset.vpc']??asset['asset.subnet']);
                const assetsFiltered = this.assetsOptions.filter( assetItem => {
                    ...
                    if (assetType === "deployment") {
                        ...
                        return assetType === assetTypeOption && assetKey === assetKeyOption;
                    }
                });
    ...
    private checkPrimaryButtonText(stepIndex: number) {
    ...
    private setPrimaryActionState(isValid: boolean): void {
        if (this.footerOptions.primaryAction) {
            this.footerOptions.primaryAction.disabled = !isValid;
        }
    }

    private validateBackStateButton(): void {
        if (this.footerOptions.secondaryAction) {
            if (this.alWizard.stepIndex === 0) {
                this.footerOptions.secondaryAction.hidden = true;
            } else {
                this.footerOptions.secondaryAction.hidden = false;
            }
        }
        this.validateStep(this.alWizard.stepIndex);
    }
    ...
    private save() {
        ...
        switch (this.formData.exposureType) {
            case 'agent':
                filters.and[0] = {"in":[{"source": "notification.agent_status"},(exposureType === 'agent'? selectedStatuses : []).map(item=>item.id)]};
                break;
            case 'appliance':
                filters.and[0] = {"in":[{"source": "notification.appliance_status"},(exposureType === 'appliance'? selectedStatuses : []).map(item=>item.id)]};
                break;
            case 'collector':
                filters.and[0] = {"in":[{"source": "notification.collector_status"},(exposureType === 'collector'? selectedStatuses : []).map(item=>item.id)]};
                break;
            default:
                break;
        }
        // always we should assign the account id filter
        filters.and[1] = {"in":[{ "source":"account.id" },[this.accountId]]};
    ...
    private stepForward(): void {
        ...
        if (this.footerOptions.primaryAction) {
            this.footerOptions.primaryAction.disabled = true;
        }
        if (this.footerOptions.secondaryAction) {
            this.footerOptions.secondaryAction.hidden = false;
        }
        ...
        if (notificationFilters && Array.isArray(notificationFilters)) {
            notificationFilters.forEach(filter => {
                if (filter.hasOwnProperty('assets') && Array.isArray(filter['assets'])) {
                    ...
                    this.assetsOptions.sort(
                        (assetA: AlSelectItem<any>, assetB: AlSelectItem<any>) => {
                            if ((assetA === undefined || assetB === undefined)) {
                                return 1;
                            } else {
                                return ((assetA.title ?? "") > (assetB.title ?? "")) ? 1 : -1;
                            }
                        }
                    );
                }
